   ...
   Components.identity      = [ones(E,1)];          % 0 for a passive resistor, 1 for an active element
    ...
    switch Components.ComponentType
        ...
        case 'atomicSwitch'
            ...
            Components.setVoltage    = ones(E,1)*1e-2; %1e-2;    % (Volt) %% sawtooth: 0.3
            Components.resetVoltage  = ones(E,1)*1e-3; %1e-3;    % (Volt) %% sawtooth: 0.01
            Components.criticalFlux  = ones(E,1)*1e-1; %1e-1;  % (Volt*sec)  %% sawtooth: 1e-4
            Components.maxFlux       = ones(E,1)*0.15; %1.5e-1 % (Volt*sec) %% sawtooth: 0.1
        case 'resistor'
