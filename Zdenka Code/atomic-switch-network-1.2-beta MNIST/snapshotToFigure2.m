function snapshotFigure = snapshotToFigure2(snapshot, contacts, connectivity, whatToPlot, axesLimits)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if whatToPlot.GraphRep
        ...
        snapshotFigure = figure('visible','off','Position', [0, 0, 1000, 800]);
        ...
        G.Edges.Weight = full(snapshot.Voltage(1:end-1));
        ...
        highlight(p, graph(adjacencyMatrix),'EdgeColor','w','LineWidth',1.5,'LineStyle','-')

        ax=plot(NaN,NaN,'b--',NaN,NaN,'w'); %plotting invisible points of desired colors
        legend(ax,'OFF switch','ON switch');        %adding the legend

        %Highlight Contacts
        highlight(p,contacts(1),'Marker', '*','MarkerSize',10,'NodeColor','g')
        highlight(p,contacts(2),'Marker', '*','MarkerSize',10,'NodeColor','r')
        ...
        if whatToPlot.Currents
            % Calculate currents:
            currents = 5e3*full(snapshot.Voltage(1:end-1)).*(snapshot.Resistance(1:end-1)); % (nA)

            %Lengths of quiver vectors
            sectionCurrentX = (p.XData(connectivity.EdgeList(2,:)) -  p.XData(connectivity.EdgeList(1,:))).*currents'/(axesLimits.CurrentArrowSacling);
            sectionCurrentY = (p.YData(connectivity.EdgeList(2,:)) -  p.YData(connectivity.EdgeList(1,:))).*currents'/(axesLimits.CurrentArrowSacling);          
        ...
        if whatToPlot.Dissipation
            p.EdgeCData = 1e9*(snapshot.Voltage(1:end-1)).^2./(snapshot.Resistance(1:end-1)); % Joule heating is V^2/R.
            cbar  = colorbar;    
        ...
        elseif whatToPlot.Lambda
            p.EdgeCData = abs(snapshot.filamentState(1:end-1));
            cbar  = colorbar;
        ...
        elseif whatToPlot.VDrop
            p.EdgeCData = abs(snapshot.Voltage(1:end-1));
            cbar  = colorbar;    
...
snapshot
        %Set title
        title(strcat(sprintf('t=%.3f (s), ', snapshot.Timestamp)));
%         ,' \sigma=', sprintf('%.2e (S)',snapshot.netC),' V=', sprintf('%.2e (V)',snapshot.netV),' I=', sprintf('%.2e (A)',snapshot.netI)));
        ...
        if whatToPlot.Contacts
            ...
            text(sourceTextPosition(1), sourceTextPosition(2), strcat('  (1) - ', num2str(contacts(1))),  'Color', 'g', 'FontSize', 16);
            text(drainTextPosition(1),  drainTextPosition(2),  strcat('  (2) - ', num2str(contacts(2))),  'Color', 'r', 'FontSize', 16);
        end
        ...
        title(strcat(sprintf('t=%.3f (s), ', snapshot.Timestamp),' \sigma=', sprintf('%.2e (S)',snapshot.netC),' V=', sprintf('%.2e (V)',snapshot.netV),' I=', sprintf('%.2e (A)',snapshot.netI)));
