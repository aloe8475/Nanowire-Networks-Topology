...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    switch Components.ComponentType
        ...
        case {'atomicSwitch', 'tunnelSwitch', 'quantCSwitch'}
            ...
            Components.setVoltage    = ones(V).*Components.setVoltage; %1e-2;    % (Volt) %% sawtooth: 0.3
            Components.resetVoltage  = ones(V).*Components.resetVoltage; %1e-3;    % (Volt) %% sawtooth: 0.01
            Components.criticalFlux  = ones(V).*Components.criticalFlux; %1e-1;  % (Volt*sec)  %% sawtooth: 1e-4
            Components.maxFlux       = ones(V).*Components.maxFlux; %1.5e-1 % (Volt*sec) %% sawtooth: 0.1
            Components.penalty       = Components.penalty; %10
            Components.boost         = Components.boost; %10
            
            Components.filamentState = ones(V) .* Components.filamentState;        % (Volt*sec)
            Components.OnOrOff       = true(V) .* (abs(Components.filamentState) > Components.criticalFlux);
