...
SimulationOptions{i}.ContactMode  = 'random';    % 'farthest' \ 'specifiedDistance' \ 'random' (the only one relevant for 'randAdjMat' (no spatial meaning)) \ 'preSet'
...
switch Connectivity{i}.WhichMatrix
    case 'nanoWires'
        ...
        %Connectivity{i}.filename = '2019-07-10-131127_asn_nw_00002_nj_00001_seed_042_avl_100.00_disp_10.00_gns_05.00_cdisp_300.00.mat';
...
switch Stimulus{i}.BiasType
    ...
    case 'AlonPulse'
        Stimulus{i}.AmplitudeOn  = 1.0;
        Stimulus{i}.AmplitudeOff = 1e-3;
        Stimulus{i}.Period       = 1; %period of the short pulses
        Stimulus{i}.LongWait     = i*0.1; %Waiting time between the first set and second set of pulses
...
%% Simulate:
if SimulationOptions{i}.takingsnapshots
    [Output{i}, SimulationOptions{i}, snapshots{i}] = simulateNetwork(Equations{i}, Components{i}, Stimulus{i}, SimulationOptions{i}, snapshotsIdx{i}); % (Ohm)
else % this discards the snaphots
    [Output{i}, SimulationOptions{i}, snapshots{i}] = simulateNetwork(Equations{i}, Components{i}, Stimulus{i}, SimulationOptions{i}); % (Ohm)
end
...
save('VariableTime.mat','snapshots','Output','SimulationOptions','Components','Connectivity','Equations','Stimulus');
%% Graphics:
