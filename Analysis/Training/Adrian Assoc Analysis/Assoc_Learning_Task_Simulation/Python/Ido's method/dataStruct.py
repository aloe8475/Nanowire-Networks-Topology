...
class network__:
    ...
    def getWireVoltage(self):
        self.wireVoltage = np.zeros((self.TimeVector.size, self.numOfWires))
        contactWires = [self.contactWires[0]-1, self.contactWires[1]-1]
        adjMat = self.adjMat
        edgeList = self.connectivity.edge_list

        for this_time in range(self.TimeVector.size):
            visited = np.full(self.numOfWires, False, dtype=bool)
            que = [contactWires[1]]
            visited[contactWires[1]] = True
            
            while len(que)>0:
                this_wire = int(que[0])
                adjWires = np.where(adjMat[this_wire,:]==1)[0]
                unvisitedAdjWires = np.array([])
                for i in adjWires:
                    if not visited[i]:
                        unvisitedAdjWires = np.append(unvisitedAdjWires, i)
                
                for i in range(len(unvisitedAdjWires)):
                    next_wire = int(unvisitedAdjWires[i])
                    if this_wire < next_wire:
                        this_junction = np.where((this_wire == edgeList[:,0])&(next_wire == edgeList[:,1]))[0][0]
                        self.wireVoltage[this_time, next_wire] = self.wireVoltage[this_time, this_wire] - \
                                                                    self.junctionVoltage[this_time, this_junction]
                    else:
                        this_junction = np.where((this_wire == edgeList[:,1])&(next_wire == edgeList[:,0]))[0][0]
                        self.wireVoltage[this_time, next_wire] = self.wireVoltage[this_time, this_wire] + \
                                                                    self.junctionVoltage[this_time, this_junction]
                que.extend(unvisitedAdjWires)
                visited[adjWires] = True
                del que[0]
...
class nanowire__:
    def __init__(self, network, index):
        ...
        if self.index != 0:
            ...
            self.isElectrode = 0
            if index in network.contactWires:
                if index == network.contactWires[0]:
                    self.isElectrode = 1
                else:
                    self.isElectrode = -1
