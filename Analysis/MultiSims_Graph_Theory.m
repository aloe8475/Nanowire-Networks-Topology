...
if strcmp(code,'Alon')
    ...
    save_directory='C:\Users\aloe8475\Documents\PhD\GitHub\CODE\Data\Figures\Explore Analysis\Single DC Pulse\';
    loadpath='C:\Users\aloe8475\Documents\PhD\GitHub\CODE\Data\Explore Analysis\Continuous DC\';
    pl{1}=[];
    pl{2}=load([loadpath 'dist4_comm_curr.mat']);
    pl{3}=load([loadpath 'dist6_comm_curr.mat']);
    pl{4}=load([loadpath 'dist8_comm_curr.mat']);
    ...
    for i = 2:length(pl)
        s{i}=scatter([pl{i}.commList],abs([pl{i}.currList]),[],cols(i,:),'filled');
        hold on;
        % h.Color='r';
        xlabel('log(Communicability)');
        %     xlim([0 8])
        ylabel('log(Current) (A)');
        [rCorrelation{i}.COMM,pCorrelation{i}.COMM]=corrcoef([pl{i}.commList],[pl{i}.currList]);
    end
    legend('PL = 4', 'PL = 6', 'PL = 8');
    set(gca,'xscale','log')
    set(gca,'yscale','log')
    %     ylim([0 1e-4]);
%     saveas(fPL,[save_directory 'Network_Current vs Communicability_Fixed_PathLength' date],'jpg');
%     print(fPL,'-painters','-dpdf','-bestfit','-r300',[save_directory 'Network_Current vs Communicability_Fixed_PathLength' date '.pdf']);
    %% Time Categories (early/mid/late/never)
    
    %     markers={'o','s','d','h'};
    
    fCOMMTime=figure('Position',[0 0 1920 1080]);
    for i = 2:length(pl)
        for j = 1:length(pl{i}.reachList)
            if pl{i}.reachList(j)==-1
                pl{i}.neverIdx(j)=1;
            end
        end
        neverIdx=logical(pl{i}.neverIdx);
        
        s2{i}=scatter([pl{i}.reachList],[pl{i}.commList],[],cols(i,:),'filled');%,markers{i});
        hold on
%         s3{i}=scatter([pl{i}.reachList(neverIdx)],[pl{i}.commList(neverIdx)],[],cols(i,:),'filled','square');
        ...
        % h.Color='r';
        xlabel('Threshold Reached (Time)');
        ylabel('COMM (Collective)');
        set(gca,'xscale','log')
                   xlim([0.15 2.5])
        set(gca,'yscale','log')
        xticks([0.2 0.4 0.6 0.8 1 1.2 1.4 1.6 1.8 2])
%         v=vline([0.6667 1.3334], {'r','r'}); %split into categories based
%         on square waves. 
%         set(v,'LineWidth', 1.5);
        hold on
...
%      saveas(fCOMMTime,[save_directory 'Communicability_Fixed_PathLength_Time_Reached_MaxI' date],'jpg');
%     print(fCOMMTime,'-painters','-dpdf','-bestfit','-r300',[save_directory 'Communicability_Fixed_PathLength_Time_Reached_MaxI' date '.pdf']);

    %% Comm categories
        fCOMMCat2=figure('Position',[0 0 1920 1080]);
        scatterTemp1=0.9+0.2*rand(1,length(pl{2}.commList)); %create random noise
        scatterTemp2=1.9+0.2*rand(1,length(pl{3}.commList)); %create random noise
        scatterTemp3=2.9+0.2*rand(1,length(pl{4}.commList)); %create random noise
        
        scatterCat.PL4=cell(1,length(pl{2}.commList));
        scatterCat.PL4(:)={'PL 4'};
        scatterCat.PL6=cell(1,length((pl{3}.commList)));
        scatterCat.PL6(:)={'PL 6'};
        scatterCat.PL8=cell(1,length(pl{4}.commList));
        scatterCat.PL8(:)={'PL 8'};
%         if equalGroups
            gscatter([scatterTemp1 scatterTemp2 scatterTemp3]',[pl{2}.commList,pl{3}.commList,pl{4}.commList]',{scatterCat.PL4{:},scatterCat.PL6{:},scatterCat.PL8{:}}',[cols(2:4,:)],[],15,'off')
         set(gca,'yscale','log');
             legend('PL = 4', 'PL = 6', 'PL = 8');   
...
%             title([num2str(length(Explore{3}{1}.GraphView.currents)) 'nw | ' num2str(length(Sim)) ' Simulations | ' num2str(Sim{3}.SimInfo.MaxV) 'V | Categories | Unequal Groups']);
%         end
        ylabel('log(COMMcollective)');
        xticks([1 2 3]);
     saveas(fCOMMCat2,[save_directory 'Communicability_Collective_PLs' date],'jpg');
    print(fCOMMCat2,'-painters','-dpdf','-bestfit','-r300',[save_directory 'Communicability_Collective_PLs' date '.pdf']);
